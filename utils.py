...
def kpi_traffic_lights(df):
    ...
    churn = 100 * (mrr.diff().fillna(0).clip(lower=0) / mrr.shift(1).replace(0, np.nan)).mean()
...
def segment_fans(df):
    ...
    k     = min(3, spend.shape[0])
    km    = KMeans(n_clusters=k, n_init='auto', random_state=12).fit(spend)
    spend['cluster'] = km.labels_
    whales = spend.sort_values('spend', ascending=False).head(int(0.05*len(spend)))
    return {
